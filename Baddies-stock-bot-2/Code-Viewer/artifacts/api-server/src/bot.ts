...
type SkinItem = { name: string };

// Load skins from catalog.json at the workspace root
let SKINS: SkinItem[] = [];
try {
  const catalogPath = path.resolve(process.cwd(), "../../../catalog.json");
  const rawCatalog: { name: string; itemType: string }[] = JSON.parse(fs.readFileSync(catalogPath, "utf8"));
  SKINS = rawCatalog
    .filter((i) => i.itemType === "Skin")
    .map((i) => ({ name: i.name }));
} catch {
  console.warn("Could not load catalog.json for skins.");
}
...
const EMOJI_SKIN = "🎨" as const;

const TICKET_CATEGORY_ID = "1477024231735951533";
...
// ── Cart ──────────────────────────────────────────────────────────────────────
type CartItem = { type: "w" | "s" | "sk"; name: string; emoji: string };
type Cart = { cats: string[]; wPage: number; skPage: number; items: CartItem[]; createdAt: number };
...
function buildItemRows(cart: Cart): ActionRowBuilder<StringSelectMenuBuilder | ButtonBuilder>[] {
  ...
  const hasSkins = cart.cats.includes("skins");
  if (hasSkins && SKINS.length > 0) {
    rows.push(
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
          .setCustomId("sk_search")
          .setLabel("🔍 Search Skins")
...
function buildCartEmbed(cart: Cart, username: string): EmbedBuilder {
  ...
  const parts: string[] = [];
  if (hasWeapons) parts.push(`${EMOJI_WEAPON} Weapons`);
  if (hasStyles)  parts.push(`${EMOJI_STYLE} Fighting Styles`);
  if (hasSkins)   parts.push(`${EMOJI_SKIN} Skins`);
  const title = parts.join(" & ") || "Stock";

  const color =
    hasWeapons && (hasStyles || hasSkins) ? Colors.Gold
    : hasWeapons ? Colors.Red
    : hasStyles  ? Colors.Purple
    : Colors.Aqua;
  ...
  if (cart.items.length === 0) {
    ...
    const lines = cart.items
      .map((it) => {
        const prefix = it.type === "w" ? EMOJI_WEAPON : it.type === "s" ? EMOJI_STYLE : EMOJI_SKIN;
        const emojiPart = it.emoji ? `${it.emoji} ` : "";
        return `${prefix} ${emojiPart}**${it.name}**`;
      })
      .join("\n");
...
export async function startBot() {
  ...
  const MOD_ROLE_ID = "1441178708311281845";
  ...
  client.on(Events.InteractionCreate, async (interaction: Interaction) => {
   try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "list") {
      ...
      const row = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
        new StringSelectMenuBuilder()
          ...
          .setPlaceholder("⚙️ Choose category (or mix)…")
          .setMinValues(1)
          .setMaxValues(3)
          .addOptions(
            new StringSelectMenuOptionBuilder()
            ...
            new StringSelectMenuOptionBuilder()
              .setLabel("Skins").setDescription("Stomps, Hoverboards, RPGs and more")
              .setValue("skins").setEmoji(EMOJI_SKIN),
          ),
      );
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "close") {
      ...
      if (!ch || !("parentId" in ch) || (ch as { parentId: string | null }).parentId !== TICKET_CATEGORY_ID) {
      ...
      await sel.update({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`Marked **${soldQty}** as sold. The listing has been updated with the new stock count.\n\nRun \`/sold\` again to update more.`)
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "list_category") {
      const sel = interaction as StringSelectMenuInteraction;
      const cats = sel.values; // ["weapons"] | ["styles"] | ["skins"] | any combo
      const key = cartKey(sel.user.id, sel.channelId);
      carts.set(key, { cats, wPage: 0, skPage: 0, items: [], createdAt: Date.now() });
    ...
    // ── Item selected from weapons, styles, or skins dropdown ────────────────
    if (
      interaction.isStringSelectMenu() &&
      (interaction.customId.startsWith("li:w:") || interaction.customId === "li:s:0" || interaction.customId.startsWith("li:sk:"))
    ) {
      const sel = interaction as StringSelectMenuInteraction;
      const key = cartKey(sel.user.id, sel.channelId);
      const cart = carts.get(key);
      ...
      for (const val of sel.values) {
        const [type, idxStr] = val.split(":") as ["w" | "s" | "sk", string];
        const idx = parseInt(idxStr);
        if (type === "w") {
          const item = WEAPONS[idx]!;
          if (!cart.items.some((it) => it.name === item.name)) {
            cart.items.push({ type: "w", name: item.name, emoji: item.emoji });
          }
        } else if (type === "s") {
          const item = FIGHTING_STYLES[idx]!;
          if (!cart.items.some((it) => it.name === item.name)) {
            cart.items.push({ type: "s", name: item.name, emoji: item.emoji });
          }
        } else if (type === "sk") {
          const item = SKINS[idx]!;
          if (item && !cart.items.some((it) => it.name === item.name)) {
            cart.items.push({ type: "sk", name: item.name, emoji: "" });
          }
        }
      }
    ...
    // ── Skin search button → show search modal ────────────────────────────
    if (interaction.isButton() && interaction.customId === "sk_search") {
      const btn = interaction as ButtonInteraction;
      const modal = new ModalBuilder()
        .setCustomId("sk_search_modal")
        .setTitle("Search Skins");
      modal.addComponents(
        new ActionRowBuilder<TextInputBuilder>().addComponents(
          new TextInputBuilder()
            .setCustomId("sk_query")
            .setLabel("Search for a skin")
            .setStyle(TextInputStyle.Short)
            .setPlaceholder("e.g. inferno, stomp, hoverboard…")
            .setRequired(true),
        ),
      );
    ...
    // ── Skin search modal submitted → show matching results ───────────────
    if (interaction.isModalSubmit() && interaction.customId === "sk_search_modal") {
      const modal = interaction as ModalSubmitInteraction;
      const key = cartKey(modal.user.id, modal.channelId!);
      const cart = carts.get(key);
      if (!cart) {
        await modal.reply({ content: "Session expired. Please run `/list` again.", ephemeral: true });
        return;
      }

      const query = modal.fields.getTextInputValue("sk_query").toLowerCase().trim();
      const matches = SKINS
        .map((item, i) => ({ item, i }))
      ...
      if (matches.length === 0) {
        await modal.reply({
          content: `No skins found matching **"${query}"**. Try a different term.`,
          ephemeral: true,
        });
        return;
      }
      ...
      await modal.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle(`${EMOJI_SKIN} Skin Search: "${query}"`)
            .setDescription(`Found **${matches.length}** result${matches.length !== 1 ? "s" : ""}. Select to add to your list.`)
        ...
        components: [
          new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
            new StringSelectMenuBuilder()
              .setCustomId("li:sk:s")
              .setPlaceholder(`${EMOJI_SKIN} Pick skin(s) to add…`)
              .setMinValues(1)
    ...
    // ── Quantities modal submitted → ask seller for accepted payment methods ─
    if (interaction.isModalSubmit() && interaction.customId === "list_quantities") {
      ...
      const lines = cart.items.map((item, i) => {
        ...
        const prefix = item.type === "w" ? EMOJI_WEAPON : item.type === "s" ? EMOJI_STYLE : EMOJI_SKIN;
        const emojiPart = item.emoji ? `${item.emoji} ` : "";
        const priceSuffix = price ? ` | 💰 ${price}` : "";
        return `${prefix} ${emojiPart}**${item.name}** — ${qty}${priceSuffix}`;
      });

      const hasWeapons = cart.items.some((i) => i.type === "w");
      const hasStyles  = cart.items.some((i) => i.type === "s");
      const hasSkins   = cart.items.some((i) => i.type === "sk");
      const color =
        hasWeapons && (hasStyles || hasSkins) ? Colors.Gold
        : hasWeapons ? Colors.Red
        : hasStyles  ? Colors.Purple
      ...
      // Store pre-built listing data and wait for payment method selection
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "lpay") {
      ...
      await sel.followUp({
        ...
        allowedMentions: { roles: ["1441178708676448267"] },
      });
      return;
    }
    ...
    if (interaction.isButton() && interaction.customId === "create_ticket") {
      ...
      // Store accepted payment methods keyed by message so ti: can look it up
      ...
      // Encode sellerId + origChannelId + origMsgId + sellerName in customId
    ...
    // ── ti: — item(s) chosen → ask quantities via modal ───────────────────
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith("ti:")) {
      const sel = interaction as StringSelectMenuInteraction;

      // customId format: ti:{sellerId}:{origChannelId}:{origMsgId}:{sellerName}
      ...
      // Fetch the original listing message to read current stock levels
      ...
      try {
        ...
        // If we can't fetch the message, fall back to no cap
      }

      // Store pending ticket with default qty of 1 for each item
      ...
      // One quantity dropdown per item, capped to that item's current stock
      // Filter out any items that have gone to 0 stock since selection
    ...
    // ── tq_submit — create ticket channel with chosen quantities ───────────
    if (interaction.isButton() && interaction.customId === "tq_submit") {
      ...
      const { sellerId, sellerName, origChannelId, origMsgId, selectedItems, quantities, acceptedPayments } = pending;
      ...
      try {
        ...
        await ticketChannel.send({
          content: `${btn.user} wants to buy from ${sellerId && sellerId !== "0" ? `<@${sellerId}>` : `**${sellerName}**`}:\n${itemLines}${paymentSuffix}`,
        ...
        await btn.reply({ content: `Failed to create ticket: ${String(err)}`, ephemeral: true });
      }
      return;
    }

    // ── ts: — seller marks as sold, updates original listing, closes ticket ──
    if (interaction.isButton() && interaction.customId.startsWith("ts:")) {
      ...
      // Parse items + quantities from the ticket opening message ("• ItemName × N")
      ...
      // Update the original listing — decrement stock, strikethrough only if it hits 0
      try {
    ...
    if (interaction.isButton() && interaction.customId.startsWith("tc_cancel:")) {
      ...
      if (btn.user.id !== sellerId && btn.user.id !== buyerId) {
        await btn.reply({ content: "Only the buyer or seller can cancel this ticket.", ephemeral: true });
        return;
      }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "list_category_single") {
