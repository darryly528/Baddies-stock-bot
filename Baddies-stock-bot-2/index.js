...
// artifacts/api-server/src/routes/control.ts
var import_express2 = require("express");
var router2 = (0, import_express2.Router)();
router2.post("/control", (req, res) => {
...
var control_default = router2;

// artifacts/api-server/src/routes/index.ts
var router3 = (0, import_express3.Router)();
router3.use(health_default);
router3.use(control_default);
...
// artifacts/api-server/src/bot.ts
var import_fs = __toESM(require("fs"), 1);
var import_path = __toESM(require("path"), 1);
var import_discord = require("discord.js");
function loadDotenv(dotenvPath = ".env") {
  try {
    const file = import_fs.default.readFileSync(import_path.default.resolve(process.cwd(), dotenvPath), "utf8");
...
var PAGE_SIZE = 24;
var EMOJI_WEAPON = "\u2694\uFE0F";
var EMOJI_STYLE = "\u{1F94B}";
var TICKET_CATEGORY_ID = "1477024231735951533";
var PAYMENT_METHODS = [
...
var pendingTickets = /* @__PURE__ */ new Map();
var pendingListings = /* @__PURE__ */ new Map();
var listingPayments = /* @__PURE__ */ new Map();
setInterval(() => {
...
function buildItemRows(cart) {
  ...
  if (hasWeapons) {
    ...
    rows.push(
      new import_discord.ActionRowBuilder().addComponents(
        new import_discord.StringSelectMenuBuilder().setCustomId(`li:w:${cart.wPage}`).setPlaceholder(`${EMOJI_WEAPON} Add weapons\u2026 (page ${cart.wPage + 1}/${totalWPages})`).setMinValues(1).setMaxValues(Math.min(25, wItems.length)).addOptions(
          wItems.map(
            (item, i) => new import_discord.StringSelectMenuOptionBuilder().setLabel(item.name).setValue(`w:${wStart + i}`).setEmoji(item.emoji)
          )
        )
      )
    );
    if (totalWPages > 1) {
      rows.push(
        new import_discord.ActionRowBuilder().addComponents(
          new import_discord.ButtonBuilder().setCustomId(`lp:w:${cart.wPage - 1}`).setLabel("\u25C0 Prev").setStyle(import_discord.ButtonStyle.Secondary).setDisabled(cart.wPage === 0),
          new import_discord.ButtonBuilder().setCustomId(`lp:w:${cart.wPage + 1}`).setLabel("Next \u25B6").setStyle(import_discord.ButtonStyle.Secondary).setDisabled(cart.wPage === totalWPages - 1)
        )
      );
    }
  }
  if (hasStyles) {
    rows.push(
      new import_discord.ActionRowBuilder().addComponents(
        new import_discord.StringSelectMenuBuilder().setCustomId("li:s:0").setPlaceholder(`${EMOJI_STYLE} Add fighting styles\u2026`).setMinValues(1).setMaxValues(Math.min(25, FIGHTING_STYLES.length)).addOptions(
          FIGHTING_STYLES.map(
            (item, i) => new import_discord.StringSelectMenuOptionBuilder().setLabel(item.name).setValue(`s:${i}`).setEmoji(item.emoji)
          )
        )
      )
    );
  ...
  rows.push(
    new import_discord.ActionRowBuilder().addComponents(
      new import_discord.ButtonBuilder().setCustomId("list_done").setLabel(
        cartCount > 0 ? `\u2705 Enter Quantities (${cartCount} item${cartCount !== 1 ? "s" : ""} selected)` : "\u2705 Select items above first"
      ).setStyle(cartCount > 0 ? import_discord.ButtonStyle.Success : import_discord.ButtonStyle.Secondary).setDisabled(cartCount === 0)
    )
  );
  return rows;
}
function buildCartEmbed(cart, username) {
  const hasWeapons = cart.cats.includes("weapons");
  const hasStyles = cart.cats.includes("styles");
  const title = hasWeapons && hasStyles ? `${EMOJI_WEAPON} Weapons & ${EMOJI_STYLE} Fighting Styles` : hasWeapons ? `${EMOJI_WEAPON} Weapons` : `${EMOJI_STYLE} Fighting Styles`;
  const embed = new import_discord.EmbedBuilder().setTitle(title).setColor(hasWeapons && hasStyles ? import_discord.Colors.Gold : hasWeapons ? import_discord.Colors.Red : import_discord.Colors.Purple).setTimestamp();
  if (cart.items.length === 0) {
    embed.setDescription("Select items from the menus below to add them to your list.");
  } else {
    const lines = cart.items.map((it) => `${it.type === "w" ? EMOJI_WEAPON : EMOJI_STYLE} ${it.emoji} **${it.name}**`).join("\n");
...
async function startBot() {
  ...
  client.once(import_discord.Events.ClientReady, async (readyClient) => {
    console.log(`Discord bot ready! Logged in as ${readyClient.user.tag}`);
    const commands = [
      new import_discord.SlashCommandBuilder().setName("list").setDescription("List your stock of Weapons and/or Fighting Styles"),
      new import_discord.SlashCommandBuilder().setName("sold").setDescription("Pick one of your recent listings to mark as sold"),
      new import_discord.SlashCommandBuilder().setName("close").setDescription("Close and delete this ticket channel")
    ].map((c) => c.toJSON());
    const rest = new import_discord.REST().setToken(token);
    try {
      await rest.put(import_discord.Routes.applicationCommands(readyClient.user.id), { body: commands });
    ...
    try {
      if (!interaction.guild) {
        if ("reply" in interaction) {
          await interaction.reply({ content: "This bot can only be used inside the server.", ephemeral: true });
        }
        return;
      }
      const memberRoles = interaction.member?.roles;
      const hasModRole = Array.isArray(memberRoles) ? memberRoles.includes(MOD_ROLE_ID) : memberRoles?.cache.has(MOD_ROLE_ID) ?? false;
      if (!hasModRole) {
        if ("reply" in interaction) {
          await interaction.reply({ content: "\u274C You need the **@mod** role to use this bot.", ephemeral: true });
        }
        return;
      }
      if (interaction.isChatInputCommand() && interaction.commandName === "list") {
        const embed = new import_discord.EmbedBuilder().setTitle("\u{1F4E6} Stock Listing").setDescription("Choose one or both categories. You can mix weapons and fighting styles.").setColor(import_discord.Colors.Blurple).setTimestamp();
        const row = new import_discord.ActionRowBuilder().addComponents(
          new import_discord.StringSelectMenuBuilder().setCustomId("list_category").setPlaceholder("\u2699\uFE0F Choose category (or both)\u2026").setMinValues(1).setMaxValues(2).addOptions(
            new import_discord.StringSelectMenuOptionBuilder().setLabel("Weapons").setDescription("Glitter Bomb, Ice Katana, Scythe and more").setValue("weapons").setEmoji(EMOJI_WEAPON),
            new import_discord.StringSelectMenuOptionBuilder().setLabel("Fighting Styles").setDescription("Heartbreaker, Angel Style and more").setValue("styles").setEmoji(EMOJI_STYLE)
          )
        );
      ...
      if (interaction.isChatInputCommand() && interaction.commandName === "sold") {
        ...
        for (const msg of fetched.values()) {
          ...
          if (!embed || embed.title !== "\u{1F4E6} Stock Listed") continue;
          ...
          if (embed.description) {
            ...
            lines.forEach((line, idx) => {
              ...
              if (options.length < 25) {
                options.push(
                  new import_discord.StringSelectMenuOptionBuilder().setLabel(name.slice(0, 100)).setDescription(`Stock: ${stock}`).setValue(`${msg.id}:${idx}:${stock}`)
                );
              }
            });
          }
          if (embed.fields.length >= 2 && !embed.fields[0].value.startsWith("~~")) {
            ...
            if (options.length < 25) {
              options.push(
                new import_discord.StringSelectMenuOptionBuilder().setLabel(itemName.slice(0, 100)).setDescription(`Stock: ${stock}`).setValue(`${msg.id}:old:${stock}`)
              );
            }
          }
        }
        ...
        await interaction.editReply({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F534} Mark as Sold").setDescription("Pick the item you sold \u2014 then choose how many.").setColor(import_discord.Colors.DarkRed).setTimestamp()
          ],
          components: [
            new import_discord.ActionRowBuilder().addComponents(
              new import_discord.StringSelectMenuBuilder().setCustomId("sold_item_pick").setPlaceholder("Choose an item\u2026").setMinValues(1).setMaxValues(1).addOptions(options)
            )
          ]
        });
        return;
      ...
      if (interaction.isStringSelectMenu() && interaction.customId === "sold_item_pick") {
        ...
        if (hasNumericStock && stock > 1) {
          ...
          for (let sold = 1; sold < stock && qtyOptions.length < 25; sold++) {
            qtyOptions.push(
              new import_discord.StringSelectMenuOptionBuilder().setLabel(`${sold} sold \u2014 ${stock - sold} remaining`).setValue(String(sold))
            );
          }
          components.push(
            new import_discord.ActionRowBuilder().addComponents(
              new import_discord.StringSelectMenuBuilder().setCustomId(`sold_qty:${msgId}:${lineRef}`).setPlaceholder("How many sold?").setMinValues(1).setMaxValues(1).addOptions(qtyOptions)
            )
          );
        }
        const soldOutLabel = hasNumericStock ? `\u{1F534} Sold Out (all ${stock} gone)` : "\u{1F534} Sold Out";
        components.push(
          new import_discord.ActionRowBuilder().addComponents(
            new import_discord.ButtonBuilder().setCustomId(`sold_out:${msgId}:${lineRef}`).setLabel(soldOutLabel).setStyle(import_discord.ButtonStyle.Danger)
          )
        );
        const itemLabel = sel.component.options.find((o) => o.value === raw)?.label ?? "this item";
        await sel.update({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F534} How many sold?").setDescription(
              `**${itemLabel}** \u2014 currently ${hasNumericStock ? `**${stock}** in stock` : `stock: ${stockStr}`}.

Choose how many sold, or mark as fully sold out.`
            ).setColor(import_discord.Colors.DarkRed).setTimestamp()
          ],
          components
        });
        return;
      ...
      if (interaction.isStringSelectMenu() && interaction.customId.startsWith("sold_qty:")) {
        ...
        if (lineRef === "old") {
          ...
          const updatedEmbed = import_discord.EmbedBuilder.from(origEmbed).spliceFields(1, 1, {
            name: "\u{1F4E6} Stock",
          ...
          await original.edit({
            embeds: [import_discord.EmbedBuilder.from(origEmbed).setDescription(lines.join("\n"))]
          });
        }
        await sel.update({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u2705 Stock Updated").setDescription(`Marked **${soldQty}** as sold. The listing has been updated with the new stock count.

Run \`/sold\` again to update more.`).setColor(import_discord.Colors.Green).setTimestamp()
          ],
          components: []
        });
        return;
      ...
      if (interaction.isButton() && interaction.customId.startsWith("sold_out:")) {
        ...
        if (lineRef === "old") {
          ...
          const soldEmbed = import_discord.EmbedBuilder.from(origEmbed).setTitle("\u{1F534} Sold Out").setColor(import_discord.Colors.DarkRed).spliceFields(0, 1, { name: origEmbed.fields[0]?.name ?? "Item", value: `~~${itemName}~~`, inline: true }).spliceFields(1, 1, { name: "\u{1F4E6} Stock", value: "~~0~~", inline: true });
          ...
          const updatedEmbed = import_discord.EmbedBuilder.from(origEmbed).setDescription(lines.join("\n")).setColor(import_discord.Colors.DarkRed);
          if (allSold) updatedEmbed.setTitle("\u{1F534} Sold Out");
          await original.edit({ embeds: [updatedEmbed] });
        }
        await btn.update({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F534} Sold Out").setDescription(`**${itemName}** has been marked as sold out.

Run \`/sold\` again to update more listings.`).setColor(import_discord.Colors.DarkRed).setTimestamp()
          ],
          components: []
        });
        return;
      ...
      if (interaction.isStringSelectMenu() && (interaction.customId.startsWith("li:w:") || interaction.customId === "li:s:0")) {
        const sel = interaction;
        const key = cartKey(sel.user.id, sel.channelId);
        const cart = carts.get(key);
        if (!cart) {
      ...
      if (interaction.isButton() && interaction.customId === "list_done") {
        ...
        const itemList = cart.items.map((it, i) => `${i + 1}. ${it.type === "w" ? EMOJI_WEAPON : EMOJI_STYLE} ${it.name}`).join("\n");
        const label = cart.items.length <= 3 ? `Quantities for: ${cart.items.map((i) => i.name).join(", ")}` : `Quantities for ${cart.items.length} items (in order listed)`;
        const modal = new import_discord.ModalBuilder().setCustomId("list_quantities").setTitle("Enter Stock Counts");
        modal.addComponents(
          new import_discord.ActionRowBuilder().addComponents(
            new import_discord.TextInputBuilder().setCustomId("items_display").setLabel("Your selected items (for reference)").setStyle(import_discord.TextInputStyle.Paragraph).setValue(itemList).setRequired(false)
          ),
          new import_discord.ActionRowBuilder().addComponents(
            new import_discord.TextInputBuilder().setCustomId("quantities").setLabel(label.slice(0, 45)).setStyle(import_discord.TextInputStyle.Short).setPlaceholder(
              cart.items.length === 1 ? "e.g. 5" : `e.g. ${cart.items.map(() => "1").join(", ")}`
            ).setRequired(true)
          ),
          new import_discord.ActionRowBuilder().addComponents(
            new import_discord.TextInputBuilder().setCustomId("prices").setLabel("Prices (optional, comma-separated)").setStyle(import_discord.TextInputStyle.Short).setPlaceholder(
      ...
      if (interaction.isModalSubmit() && interaction.customId === "list_quantities") {
        ...
        const lines = cart.items.map((item, i) => {
          ...
          const priceSuffix = price ? ` | \u{1F4B0} ${price}` : "";
        ...
        const color = hasWeapons && hasStyles ? import_discord.Colors.Gold : hasWeapons ? import_discord.Colors.Red : import_discord.Colors.Purple;
        ...
        await modal.reply({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F4B3} Which payment methods do you accept?").setDescription("Choose all that apply \u2014 buyers will see these on your listing.").setColor(import_discord.Colors.Blurple).setTimestamp()
          ],
          components: [
            new import_discord.ActionRowBuilder().addComponents(
              new import_discord.StringSelectMenuBuilder().setCustomId("lpay").setPlaceholder("Select payment method(s)\u2026").setMinValues(1).setMaxValues(PAYMENT_METHODS.length).addOptions(
                PAYMENT_METHODS.map(
                  (pm) => new import_discord.StringSelectMenuOptionBuilder().setLabel(pm.label).setValue(pm.value).setEmoji(pm.emoji)
                )
              )
            )
          ],
      ...
      if (interaction.isStringSelectMenu() && interaction.customId === "lpay") {
        ...
        const selectedPayments = sel.values.map((v) => PAYMENT_METHODS.find((pm) => pm.value === v)).filter((pm) => !!pm);
        const paymentLine = "\u{1F4B3} " + selectedPayments.map((pm) => `<:${pm.emoji.name}:${pm.emoji.id}> ${pm.label}`).join(" | ");
        const description = `@${pl.username}'s Stock

${pl.lines.join("\n")}

${paymentLine}`;
        const resultEmbed = new import_discord.EmbedBuilder().setTitle("\u{1F4E6} Stock Listed").setDescription(description).setColor(pl.color).setThumbnail(pl.avatarURL).setFooter({ text: `Listed by ${pl.username} \u2022 ${pl.userId}`, iconURL: pl.avatarURL }).setTimestamp();
        await sel.update({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F4E6} Listed!").setDescription("Run `/list` again to add more stock.").setColor(import_discord.Colors.Green).setTimestamp()
          ],
          components: []
        });
        await sel.followUp({
          ...
          components: [
            new import_discord.ActionRowBuilder().addComponents(
              new import_discord.ButtonBuilder().setCustomId("create_ticket").setLabel("Create Ticket").setStyle(import_discord.ButtonStyle.Primary)
            )
          ],
      ...
      if (interaction.isButton() && interaction.customId === "create_ticket") {
        ...
        const itemNames = itemLines.filter((line) => !line.includes("~~")).map((line) => line.match(/\*\*(.+?)\*\*/)?.[1]).filter((n) => !!n);
        const footerText = embed.footer?.text ?? "";
        const sellerIdMatch = footerText.match(/•\s*(\d+)$/);
        const sellerId = sellerIdMatch?.[1] ?? "0";
        const acceptedPayments = descLines.find((l) => l.startsWith("\u{1F4B3}")) ?? "";
        ...
        await btn.reply({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F6D2} What do you want to buy?").setDescription(`Choose the item(s) you want from **${sellerName}**'s stock:`).setColor(import_discord.Colors.Blurple).setTimestamp()
          ],
          components: [
            new import_discord.ActionRowBuilder().addComponents(
              new import_discord.StringSelectMenuBuilder().setCustomId(selectId).setPlaceholder("Select item(s)\u2026").setMinValues(1).setMaxValues(Math.min(4, itemNames.length)).addOptions(
                itemNames.slice(0, 25).map(
                  (name) => new import_discord.StringSelectMenuOptionBuilder().setLabel(name).setValue(name)
                )
              )
            )
          ],
      ...
      if (interaction.isStringSelectMenu() && interaction.customId.startsWith("ti:")) {
        ...
        if (availableCapped.length === 0) {
          await sel.update({
            embeds: [
              new import_discord.EmbedBuilder().setTitle("\u274C Items No Longer Available").setDescription("All selected items have sold out. Please try again.").setColor(import_discord.Colors.Red).setTimestamp()
            ],
            components: []
          });
        ...
        const qtyRows = availableCapped.map(
          (name, idx) => {
            const maxQty = Math.min(Math.max(stockMap.get(name) ?? 20, 1), 25);
            const qtyOptions = Array.from(
              { length: maxQty },
              (_, i) => new import_discord.StringSelectMenuOptionBuilder().setLabel(`${i + 1}`).setValue(`${i + 1}`).setDefault(i === 0)
            );
            return new import_discord.ActionRowBuilder().addComponents(
              new import_discord.StringSelectMenuBuilder().setCustomId(`tqd:${idx}`).setPlaceholder(`Qty for: ${name.slice(0, 60)} (max ${maxQty})`).setMinValues(1).setMaxValues(1).addOptions(qtyOptions)
            );
          }
        );
        qtyRows.push(
          new import_discord.ActionRowBuilder().addComponents(
            new import_discord.ButtonBuilder().setCustomId("tq_submit").setLabel("\u2705 Create Ticket").setStyle(import_discord.ButtonStyle.Success)
          )
        );
        await sel.update({
          embeds: [
            new import_discord.EmbedBuilder().setTitle("\u{1F522} How many of each?").setDescription(
              availableCapped.map((name, i) => `${i + 1}. **${name}**`).join("\n") + "\n\nSelect a quantity for each item, then click **\u2705 Create Ticket**."
            ).setColor(import_discord.Colors.Blurple).setTimestamp()
          ],
          components: qtyRows
        });
        return;
      ...
      if (interaction.isButton() && interaction.customId === "tq_submit") {
        ...
        const channelName = selectedItems.slice(0, 3).join("-").toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-").slice(0, 100);
        const permissionOverwrites = [
          { id: guild.roles.everyone.id, deny: [import_discord.PermissionFlagsBits.ViewChannel] },
          {
            id: btn.user.id,
            allow: [import_discord.PermissionFlagsBits.ViewChannel, import_discord.PermissionFlagsBits.SendMessages, import_discord.PermissionFlagsBits.ReadMessageHistory]
          }
        ];
        ...
        try {
          ...
          const itemLines = selectedItems.map((name, i) => `\u2022 ${name} \xD7 ${quantities[i] ?? 1}`).join("\n");
          const paymentSuffix = acceptedPayments ? `
${acceptedPayments}` : "";
          const soldBtn = new import_discord.ButtonBuilder().setCustomId(`ts:${sellerId}:${origChannelId}:${origMsgId}`).setLabel("\u2705 Sold").setStyle(import_discord.ButtonStyle.Success);
          const cancelBtn = new import_discord.ButtonBuilder().setCustomId(`tc_cancel:${sellerId}:${btn.user.id}`).setLabel("\u274C Cancel").setStyle(import_discord.ButtonStyle.Danger);
          await ticketChannel.send({
            content: `${btn.user} wants to buy from ${sellerId && sellerId !== "0" ? `<@${sellerId}>` : `**${sellerName}**`}:
${itemLines}${paymentSuffix}`,
            components: [new import_discord.ActionRowBuilder().addComponents(soldBtn, cancelBtn)]
          });
          await btn.update({
            embeds: [
              new import_discord.EmbedBuilder().setTitle("\u2705 Ticket Created").setDescription(`Your ticket is ready: ${ticketChannel}${paymentSuffix}`).setColor(import_discord.Colors.Green).setTimestamp()
            ],
            components: []
          });
        } catch (err) {
      ...
      if (interaction.isButton() && interaction.customId.startsWith("ts:")) {
        ...
        const soldItems = btn.message.content.split("\n").filter((l) => l.startsWith("\u2022 ")).map((l) => {
          const m = l.match(/^• (.+?) × (\d+)$/);
