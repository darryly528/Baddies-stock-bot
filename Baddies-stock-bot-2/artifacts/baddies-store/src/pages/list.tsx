...
export default function ListPage() {
  ...
  async function handleBuyViaDiscord(listingId: string, inviteUrl: string) {
    ...
    try {
      await fetch(`/api/listings/${listingId}/notify-seller`, {
        method: "POST",
        credentials: "include",
      });
    } catch {
      // best-effort — still open Discord even if notify fails
    } finally {
      setNotifyingId(null);
    }
                          ...
                          <div className="flex items-center gap-2">
                            {!isOwner && inviteUrl && (
                              <button
                                ...
                                disabled={notifyingId === listing.id}
                                className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-[#5865F2]/20 text-[#5865F2] hover:bg-[#5865F2]/30 border border-[#5865F2]/30 transition-colors whitespace-nowrap disabled:opacity-60 disabled:cursor-wait"
                              >
                                {notifyingId === listing.id ? (
